...
export default {
  data: new SlashCommandBuilder()
    .setName("radio")
    .setDescription(
      "Radio mode adds related songs to the queue, songs that are already in the queue will not be added",
    )
    .addSubcommand(
      ...
      new SlashCommandSubcommandBuilder()
        ...
        .addIntegerOption(
          new SlashCommandIntegerOption()
            ...
            .setMinValue(1)
            .setDescription(
              "Limits the max number of related songs that can be added at once",
            ),
        )
        .addIntegerOption(
          new SlashCommandIntegerOption()
        ...
        .addBooleanOption(
          new SlashCommandBooleanOption()
            .setName("unique")
            .setDescription(
              "Whether related songs that are already in queue should be added",
            ),
        ),
    ),
  execute(context: CommandContext) {
    ...
    switch (interaction.options.getSubcommand()) {
      case "info": {
        ...
        fragmentedMessages.push(
          `Radio is ${music.options.radio.isOn ? "on" : "off"}`,
        );
        fragmentedMessages.push(
          `Related songs limit: ${music.options.radio.relatedSongsLimit}`,
        );
        fragmentedMessages.push(
          `Related songs randomness: ${music.options.radio.relatedSongsRandomness}`,
        );
        fragmentedMessages.push(
          `Suggest unique songs: ${music.options.radio.suggestUniqueSongs}`,
        );
      ...
      case "options": {
        ...
        if (limit !== null) {
          fragmentedMessages.push(
            `Related songs limit: ${music.options.radio.relatedSongsLimit} -> ${limit}`,
          );
        ...
        if (randomness !== null) {
          fragmentedMessages.push(
            `Related songs randomness: ${music.options.radio.relatedSongsRandomness} -> ${randomness}`,
          );
        ...
        if (unique !== null) {
          fragmentedMessages.push(
            `Suggest unique songs: ${music.options.radio.suggestUniqueSongs} -> ${unique}`,
          );
